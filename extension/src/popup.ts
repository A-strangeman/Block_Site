import { STORAGE_KEYS, normalizeDomain, type AuthState, type BlockedSite } from './shared';
...
async function loadState(): Promise<void> {
  ...
  const auth = (result[STORAGE_KEYS.auth] ?? {}) as AuthState;
  tokenInput.value = auth.token ?? '';
  friendInput.value = (result[STORAGE_KEYS.friend] ?? '') as string;
}

async function saveAuth(token: string): Promise<void> {
  await chrome.storage.local.set({ [STORAGE_KEYS.auth]: { token } satisfies AuthState });
}

async function saveFriend(value: string): Promise<void> {
  await chrome.storage.local.set({ [STORAGE_KEYS.friend]: value });
}

async function saveBlockedSite(domain: string): Promise<void> {
  const result = await chrome.storage.local.get(STORAGE_KEYS.blockedSites);
  const blockedSites = ((result[STORAGE_KEYS.blockedSites] ?? []) as BlockedSite[]).filter(Boolean);
...
document.getElementById('unlock')?.addEventListener('click', async () => {
  ...
  try {
    ...
    await chrome.storage.local.set({ [STORAGE_KEYS.auth]: { token, unlockUntil: Date.now() + 5 * 60 * 1000 } satisfies AuthState });
